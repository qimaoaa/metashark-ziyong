...
namespace Jellyfin.Plugin.MetaShark.Providers
{
    ...
    public abstract class BaseProvider
    {
        ...
        protected async Task<TMDbLib.Objects.Search.TvSeasonEpisode?> GetEpisodeAsync(int seriesTmdbId, int? seasonNumber, int? episodeNumber, string displayOrder, string? language, string? imageLanguages, CancellationToken cancellationToken)
        {
            ...
            if (TmdbEpisodeGroupMapping.TryGetGroupId(this.Config.TmdbEpisodeGroupMap, seriesId, out var groupId))
            {
        ...
        public async Task<HttpResponseMessage> GetImageResponse(string url, CancellationToken cancellationToken)
        {
            ...
            if (url.Contains("doubanio.com", StringComparison.OrdinalIgnoreCase))
            {
                // 相对链接补全
                if (!url.StartsWith("http", StringComparison.OrdinalIgnoreCase) && Plugin.Instance != null)
                {
                    url = Plugin.Instance.GetLocalApiBaseUrl().ToString().TrimEnd('/') + url;
                }

                // 包含了代理地址的话，从url解析出原始豆瓣图片地址
                if (url.Contains("/proxy/image", StringComparison.Ordinal))
                {
            ...
            else
            {
                this.Log("GetImageResponse url: {0}", url);
                using var client = this.HttpClientFactory.CreateClient();
                return await client.GetAsync(new Uri(url), cancellationToken).ConfigureAwait(false);
            }
        }
        ...
        protected async Task<string?> GuessByDoubanAsync(ItemLookupInfo info, CancellationToken cancellationToken)
        {
            ...
            if (this.Config.EnableDoubanAvoidRiskControl)
            {
        ...
        public async Task<string?> GuestDoubanSeasonByYearAsync(string seriesName, int? year, int? seasonNumber, CancellationToken cancellationToken)
        {
            ...
            if (item != null && !string.IsNullOrEmpty(item.Sid))
            {
                ...
                var nameIndexNumber = this.ParseChineseSeasonNumberByName(item.Name);
        ...
        {
            var baseUrl = Plugin.Instance?.GetLocalApiBaseUrl().ToString() ?? string.Empty;
            var proxyBaseUrl = this.Config.DoubanImageProxyBaseUrl?.ToString();
        ...
        private string GetBaseUrl()
        {
            ...
            if (Plugin.Instance != null && this.HttpContextAccessor.HttpContext != null)
            {
                return Plugin.Instance.GetApiBaseUrl(this.HttpContextAccessor.HttpContext.Request).ToString();
            }

            // 自动扫描刷新时，直接使用本地地址(127.0.0.1)
            return Plugin.Instance?.GetLocalApiBaseUrl().ToString() ?? string.Empty;
        }

        protected void Log(string? message, params object?[] args)
        {
            this.Logger.LogInformation($"[MetaShark] {message}", args);
        }
        ...
        protected string GetDoubanPoster(DoubanSubject subject)
        {
            ...
            var url = this.Config.EnableDoubanLargePoster ? subject.ImgLarge : subject.ImgMiddle;
        ...
        public Task<HttpResponseMessage> GetImageResponse(Uri url, CancellationToken cancellationToken)
        {
            return Task.FromException<HttpResponseMessage>(new NotImplementedException());
        }
    }
}
